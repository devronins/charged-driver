...
const OnlineOffline = () => {
    ...
    const toggleSwitch = () => {
        animateScale();
        dispatch(editDriver({ driverDetails: { ...driverDetails,is_online: driverDetails?.is_online ? false : true } }))
    };

    return (
        <View className="absolute bottom-8 z-10 self-center">
            <Animated.View className="flex-row items-center py-2.5 px-4 rounded-full border border-black/10 shadow-md bg-white gap-4" style={{ transform: [{ scale: scaleAnim }] }}>
                <View className='flex items-center justify-center'>
                    <Text className={twMerge('text-lg font-semibold', driverDetails?.is_online ? 'text-tertiary-300' : 'text-red-400')}>
