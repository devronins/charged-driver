...
const schema = yup.object().shape({
    ...
    car_year: yup.number().required("Vehicle Year is required").min(1900, "Vehicle Year must be 1900 or later").max(new Date().getFullYear(), `Vehicle Year cannot be in the future`),
    car_type: yup.string().required("Vehicle Type is required").oneOf(["suv", "regular", "electric"], "Vehicle Type should be one of the following: suv, regular, or electric.")
});

const VehicleInfromationForm = () => {
    const { vehicleDetails, vehicleDetailsLoading, isEditMode } = useTypedSelector(state => state.Vehicle);
                ...
                <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                        <InputField
                            ...
                            value={isEditMode ? watch('car_model') : value ? watch('car_model') : 'Not Provided'}
                ...
                <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                        <InputField
                            ...
                            value={isEditMode ? watch('license_plate') : value ? watch('license_plate') : 'Not Provided'}
                            onChangeText={onChange}
                            editable={!isEditMode ? false : vehicleDetailsLoading ? false : true}
                ...
                <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                        <InputField
                            ...
                            label="Vehicle Color"
                            placeholder="eg. Red"
                            value={isEditMode ? watch('car_color') : value ? watch('car_color') : 'Not Provided'}
                            onChangeText={onChange}
                            editable={!isEditMode ? false : vehicleDetailsLoading ? false : true}
                    ...
                    render={({ field: { onChange, value } }) => (
                        < InputField
                           keyboardType="numeric"
                            ...
                            label="Vehicle Year"
                            placeholder="eg. 2024"
                            value={ isEditMode ? watch('car_year')?.toString() : value ? watch('car_year')?.toString() : 'Not Provided'}
                            onChangeText={onChange}
                            editable={!isEditMode ? false : vehicleDetailsLoading ? false : true}
                ...
                <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                        <InputField
                            ...
                            label="Vehicle Type"
                            placeholder="eg. Suv"
                            value={isEditMode ? watch('car_type') : value ? watch('car_type') : 'Not Provided'}
                            onChangeText={onChange}
                            editable={!isEditMode ? false : vehicleDetailsLoading ? false : true}
